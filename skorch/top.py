    ...
    def fit(self, X, Y=None, lr=0.01, cv=5, batch_size=32, shuffle=True, epoches=10,early_stop=None):
        ...
        for train_indexes, test_indexes in kfold.split(X, Y):
            ...
            for epoch in range(1, epoches + 1):
                ...
                print('cv {} epoch {} train loss:{:.4f} train acc:{:.4f} '
...
class BaseRegressor(RegressorMixin):
    def __init__(self, net: nn.Module, weight=None, device=None, loss_func=None, optim=None, worker_num=-1):
        super().__init__()
        ...
        self.loss_func = loss_func or F.mse_loss
        self.optimizer_func = optim or optimizer.SGD
    ...
    def score(self, X, Y=None, weight=None):
        ...
        if Y.dtype != np.float32:
            Y = Y.astype(np.float32)
        pred = self.predict(X)
        return r2_score(pred.flatten(), Y.flatten())
